...
const App = (function() {
  ...
  function replaceEmojisWithIcons() {
    ...
    elements.forEach(el => {
      ...
      const fallbackEmoji = el.getAttribute('data-emoji') || '📌';
  ...
  function renderApp(state) {
    ...
    // Actualizar XP y nivel (progreso dentro del nivel actual)
    setEl('levelBadge', `Nv. ${levelInfo.level}`);
    setEl('levelName', levelInfo.name);
    setEl('currentXp', levelInfo.xpInLevel || 0);
    setEl('nextXp', levelInfo.xpNeeded || nextXp);
    setEl('sidebarXp', `${levelInfo.xpInLevel}/${levelInfo.xpNeeded} XP`);
    setEl('mobileXp', `${levelInfo.xpInLevel}/${levelInfo.xpNeeded} XP`);
    ...
    setEl('pXp', `${levelInfo.xpInLevel}/${levelInfo.xpNeeded} XP`);
    setEl('pLevel', levelInfo.level);
    setEl('pLessons', state.totalLessons || 0);
    setEl('pStreak', state.streakDays || 0);

    // Avatar según nivel - actualizar data-icon
    const avatarLevel = Math.min(levelInfo.level - 1, 9);
    const avatarEl = document.getElementById('sidebarAvatar');
    if (avatarEl) {
      avatarEl.setAttribute('data-icon', 'avatar-' + avatarLevel);
      avatarEl.setAttribute('data-emoji', '🧑');
    }
    const profileAvatarEl = document.getElementById('profileAvatar');
    if (profileAvatarEl) {
      profileAvatarEl.setAttribute('data-icon', 'avatar-' + avatarLevel);
    }
    ...
    // Actualizar "Continuar aprendiendo" con datos reales
    updateContinueCard(state);
  ...
  function updateContinueCard(state) {
    const continueCard = document.getElementById('continueCard');
    if (!continueCard) return;

    const moduleProgress = state.moduleProgress || {};
    const completedLessons = state.completedLessons || [];
    const currentModuleId = state.currentModule || 'pc_basico';
    
    // Obtener módulo actual
    const module = Modules.getModule(currentModuleId);
    if (!module) return;

    // Calcular progreso del módulo
    const progress = moduleProgress[currentModuleId] || 0;
    ...
    if (module.lessons) {
      for (let i = 0; i < module.lessons.length; i++) {
        if (!completedLessons.includes(module.lessons[i].id)) {
          nextLesson = module.lessons[i];
          lessonNum = i + 1;
          break;
        }
      }
      // Si todas completadas, usar la última
      if (!nextLesson && module.lessons.length > 0) {
        nextLesson = module.lessons[module.lessons.length - 1];
        lessonNum = module.lessons.length;
      }
    }

    // Actualizar contenido de la tarjeta
    const iconEl = continueCard.querySelector('.continue-icon');
    const moduleEl = continueCard.querySelector('.continue-module');
    const lessonEl = continueCard.querySelector('.continue-lesson');
    const progressFill = continueCard.querySelector('.progress-fill');
    const btn = continueCard.querySelector('.btn');

    if (iconEl) iconEl.innerHTML = getIcon(module.id, module.icon || '📚');
    if (moduleEl) moduleEl.textContent = module.name || 'Módulo';
    if (lessonEl) {
      if (nextLesson) {
        lessonEl.textContent = `Lección ${lessonNum}: ${nextLesson.title}`;
      } else if (progress >= 100) {
        lessonEl.textContent = '¡Módulo completado!';
      } else {
        lessonEl.textContent = 'Continúa aprendiendo';
      }
    }
  ...
  function getIcon(name, fallback) {
    return '<img src="assets/icons/' + name + '.png" class="ui-icon">';
  }
    ...
    ROADMAP_ORDER.forEach((modId, index) => {
      ...
      html += `
        <div class="${nodeClass}" ${clickHandler} data-module="${modId}">
          <div class="node-icon">${getIcon(modId, moduleData.icon)}</div>
...
function openModule(moduleId) {
  ...
  if (titleEl) titleEl.innerHTML = `${getIcon(module.id, module.icon)} ${module.name}`;
...
// Función global para obtener iconos
window.getIcon = function(name, fallback) {
  return '<img src="assets/icons/' + name + '.png" class="ui-icon">';
};