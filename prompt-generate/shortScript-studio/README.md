...
- 🔊 **Audios TTS** usando Puter.js (gratis, voces neuronales) o Web Speech API
- 🎤 **Voces neuronales gratuitas** - sin API keys requeridas
...
## 🚀 Cómo usar la generación de audio

### ¿Cómo funciona?

La generación de audio de ShortScript Studio está diseñada para ser **100% gratuita** y no requiere API keys:

1. **Puter.js (Primera opción)**: Usa voces neuronales de alta calidad de forma gratuita. No necesitas registrarte ni pagar nada.

2. **Web Speech API (Fallback)**: Si Puter.js no está disponible, usa la voz del navegador (funcional en todos los navegadores modernos).

3. **Kokoro TTS (Alternativa local)**: Modelo TTS open-source que se ejecuta directamente en tu navegador.
...
- **Puter.js**: 20 solicitudes por minuto (suficiente para uso normal)
- **Web Speech API**: Disponible en todos los navegadores, calidad variable
- **Kokoro TTS**: Puede ser lento en dispositivos con recursos limitados

### ¿Necesito API keys?

**No**, puedes usar la herramienta completamente gratis. Las voces neuronales de Puter.js son gratuitas y de alta calidad.

**Opcional**: Si deseas mayor calidad o voces específicas, puedes configurar:
- **OpenAI**: Para generación de guiones con IA
- **ElevenLabs**: Para voces TTS premium
...
### Con ElevenLabs (voces de alta calidad - opcional)
...
**Nota**: Sin API keys, la herramienta usa Puter.js gratuitamente con voces neuronales de alta calidad.
...
### Cambiar la voz de ElevenLabs (opcional)
...
Puter.js ya viene integrado y no requiere configuración. Las voces neuronales se usan automáticamente.
...
1. **Verifica tu conexión a internet**: Puter.js requiere conexión
2. **Abre la consola (F12)**: Revisa los mensajes de error en la pestaña Console
3. **Intenta de nuevo**: A veces las voces neuronales pueden tardar un poco
4. **Usa el fallback**: Si todo falla, la voz del navegador siempre funciona
...
### Los guiones no se generan

- Sin API key de OpenAI: La herramienta usa plantillas locales
- Con API key: Verifica que sea una clave válida de OpenAI
