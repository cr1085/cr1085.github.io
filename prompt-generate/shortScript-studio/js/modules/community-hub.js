 ...
 * Uses Supabase if available; otherwise shows fallback examples.
 */

export class CommunityHub {
  constructor(supabaseClient, toast) {
  ...
  async load() {
    ...
    // If no Supabase or empty, show example prompts
    if (!prompts.length) {
  ...
  async share({ name, niche, prompt }) {
    if (!this.db.isReady()) {
      this.toast.show('Configura Supabase para compartir prompts', 'warn');
      return;
    }
    try {
