 ...
 * - Uses Puter.js (free, no API key) as primary method
 * - Falls back to OpenAI API if key is available
 * - Falls back to local template generator only if both fail
 */
...
export class ScriptGenerator {
  ...
  async generate(prompt, params) {
    // 1. Try Puter.js (free, no API key, real AI)
    try {
      return await this._generateWithPuter(prompt);
    } catch (err) {
      this.toast.show(`⚠ Puter.js falló: ${err.message}. Intentando OpenAI...`, 'warn', 4000);
    }
    ...
    this.toast.show('⚠ Usando generador local (contenido de ejemplo)', 'warn', 4000);
    return this._generateLocally(params);
  }

  /** ── Puter.js (free AI, no API key) ── */
  async _generateWithPuter(prompt) {
    if (typeof puter === 'undefined') {
      throw new Error('Puter.js no está cargado');
    }

    // Puter chat returns the raw text response
    const raw = await puter.ai.chat(prompt, { model: 'gpt-4o-mini', stream: false });

    if (!raw || typeof raw !== 'string') {
      throw new Error('Respuesta vacía de Puter.js');
    }
  ...
  /** ── Local template-based generator (fallback only) ── */
