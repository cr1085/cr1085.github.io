...
export class UIController {
  ...
  if (loading) {
    btn.disabled = true;
    label.textContent = 'GENERANDO…';
    // El loader ya lo muestra el script de partículas via window.showLoader
  } else {
    btn.disabled = false;
    label.textContent = 'GENERAR GUIONES';
    icon.innerHTML = `<path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/>`;
  ...
  /** ── Render script cards ── */
  renderScripts(scripts, { onTabChange, onCopyScript, onPlayAudio }, audios = {}) {
    const grid = document.getElementById('scripts-grid');
    grid.innerHTML = '';

    scripts.forEach((script, i) => {
      const hasAudio = !!audios[i];
      const card = this._buildScriptCard(script, i, { onTabChange, onCopyScript, onPlayAudio, hasAudio });
      grid.appendChild(card);
      if (hasAudio) {
        this.attachAudioPlayer(i, audios[i], true);
      }
    });
  }

  /** Build a single script card */
  _buildScriptCard(script, idx, { onTabChange, onCopyScript, onPlayAudio, hasAudio }) {
    ...
    header.innerHTML = `
      ...
      ${hasAudio ? '<span class="text-green text-xs ml-2" title="Audio disponible">🔊</span>' : ''}
  ...
  attachAudioPlayer(idx, blob, showOptions = false) {
    ...
    pane.innerHTML = `
      <div class="audio-player flex-col">
        <div class="flex items-center gap-3 w-full">
          <span class="text-xs text-ash font-mono">Guion ${idx + 1}</span>
          <audio controls src="${url}" class="flex-1"></audio>
          <a href="${url}" download="audio_${String(idx+1).padStart(2,'0')}.${ext}" class="btn-secondary text-xs py-1 px-3">⬇</a>
        </div>
        ${showOptions ? `
        <div class="flex gap-2 mt-2">
          <button class="btn-primary text-xs py-1 px-3 regenerate-audio-btn" data-idx="${idx}">🔄 Nuevo audio</button>
          <button class="btn-danger text-xs py-1 px-3 remove-audio-btn" data-idx="${idx}">🗑️ Eliminar</button>
        </div>
        ` : ''}
      </div>
    `;
