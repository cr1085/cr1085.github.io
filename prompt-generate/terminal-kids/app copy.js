...
// ══════════════════════════════════════════════
// CONFIGURACIÓN
// Reemplaza con tu API key real (usa variables de entorno en producción)
// ══════════════════════════════════════════════
const CONFIG = {
...
function buildSystemPrompt() {
  ...
  return `Eres Lumi, un tutor virtual educativo amigable, paciente y motivador para estudiantes.
...
REGLAS IMPORTANTES:
❌ NUNCA dar contenido violento, inapropiado o que no sea educativo
❌ NUNCA frustrar al estudiante
❌ NUNCA dar respuestas muy largas (máximo 4-5 párrafos cortos)
✅ SIEMPRE conectar el tema con ejemplos reales y cotidianos
✅ SIEMPRE ofrecer practicar con un ejercicio después de explicar
...
async function getAIResponse(userMessage) {
  ...
  try {
    ...
    const aiText = data.choices?.[0]?.message?.content || "Lo siento, tuve un problema. ¡Intenta de nuevo! 😊";
